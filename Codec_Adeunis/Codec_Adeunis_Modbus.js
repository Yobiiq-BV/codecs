...
Object.prototype.readInt32LE = function (offset) {
    var buffer = this;
...
Object.prototype.readUInt32LE = function (offset) {
    var buffer = this;
...
Object.prototype.readFloatLE = function (offset) {
    var buffer = this;
    var value = ((buffer[offset + 3] << 24) | (buffer[offset + 2] << 16) | (buffer[offset + 1] << 8) | buffer[offset]) >>> 0;
    return new Float32Array(new Uint32Array([value]).buffer)[0];
};
...
(function (codec) {
    var Modbus0x44Parser = (function () {
        ...
        Modbus0x44Parser.prototype.parseFrame = function (payload, configuration, network) {
            var appContent = { type: '0x44 Modbus data (uint32)' };
            var registers = [];
            for (var offset = 2; offset < payload.length; offset += 4) {
                switch(offset)
                {
                    case 2:
                        registers.push(payload.readUInt32LE(offset));
                        break;
                    default:
                        registers.push(parseFloat(payload.readFloatLE(offset).toFixed(5)));
                        break;
                }
            }
...
(function (codec) {
    var Modbus0x5fParser = (function () {
        ...
        Modbus0x5fParser.prototype.parseFrame = function (payload, configuration, network) {
            var appContent = { type: '0x5f Modbus data (float)' };
            var registers = [];
            for (var offset = 2; offset < payload.length; offset += 4) {
                registers.push(parseFloat(payload.readFloatLE(offset).toFixed(3)));
            }
            appContent['registerValues'] = registers;
            return appContent;
        };
...
function decodeUplink(input) {
    ...
    var decoded = decoder.decode(input.bytes);
    var deviceTimestamp = parseInt(Date.now()/1000);
    var measurements = [];
    if(decoded.registerValues)
    {
        for(var i=0; i<decoded.registerValues.length; i=i+1)
        {
            if(decoded.registerValues[i] === undefined || 
                decoded.registerValues[i] === null ||
                isNaN(decoded.registerValues[i]) ||
                decoded.registerValues[i] === 4294967295)
            {
